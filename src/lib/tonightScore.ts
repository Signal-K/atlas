// Pure scoring function for "is this night worth going outside" -- no React,
// no I/O -- callers gather conditions from weather.ts and moon illumination,
...
export function scoreTonight(conditions: TonightConditions): TonightScoreResult {
  ...
  if (conditions.cloudCoverPct >= 60) {
    points -= 40
    reasons.push(`Mostly cloudy (${Math.round(conditions.cloudCoverPct)}% cover)`)
